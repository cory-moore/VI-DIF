...
"
vi <- read.csv("data/vi.cult.csv", header=TRUE)
...
anc.items.names.R <- itemnames.R[c(5,6)] 
test.items.R <- c(3,8)
...
## Investigative
(dif.anchor.I <- DIF(model_anchor.I, c('a1','d1','d2','d3'), items2test = test.items.I, plotdif = TRUE))
## use the optional function to table the output
...
itemplot(model_anchor.I, 5) # further investigate item with DF
...
expected.test.plot.I$legend$top$args$key$text[[1]] <- c('Focal', 'Reference')
expected.test.plot.I
